...
function CardMenu({ note, btnRef, onClose, onView, onEdit, onDuplicate, onDelete, onRestore, onShare }) {
  ...
  useEffect(() => {
    ...
    const menuW = 172
    const menuH = note.isTrashed ? 148 : 156  // +36px for share item

    let left = r.right - menuW
    let top  = r.bottom + 6
    if (left < 6) left = 6
    if (left + menuW > window.innerWidth  - 6) left = window.innerWidth  - menuW - 6
    if (top  + menuH > window.innerHeight - 6) top  = r.top - menuH - 6
    setPos({ top, left })
  }, [])
  ...
  const menuItem = (fn, icon, label, danger = false) => (
    <button
  ...
  return createPortal(
    <div
      ...
      style={{
        position: 'fixed', top: pos.top, left: pos.left, width: 172,
        zIndex: 9999,
    ...
    >
      ...
      {!note.isTrashed && menuItem(onShare, note.isShared ? '🔗' : '↗', note.isShared ? 'Manage share' : 'Share note')}
...
function NoteCard({ note, viewMode, onView, onEdit, onDelete, onRestore, onDuplicate, onTogglePin, onToggleFavorite, onShare, index = 0 }) {
  ...
  return (
    <div
      ...
      // Entrance animation via inline style on first render only
      // hover transform/shadow via separate transition (no animation reset)
      style={{
        position: 'relative',
        background: 'var(--bg-card)',
        ...
        // Entrance: set via CSS custom property so React doesn't re-run it on re-render
        '--delay': `${delayMs}ms`,
      }}
          ...
          >
            <Tooltip text={note.isFavorite ? 'Remove favourite' : 'Add to favourites'} position="top">
              <button
                ...
                style={{ width:24,height:24,display:'flex',alignItems:'center',justifyContent:'center',borderRadius:5,border:'none',background:'transparent',cursor:'pointer',fontSize:13,color:note.isFavorite?'var(--warm)':'var(--text-tertiary)',transition:'color 0.15s ease, transform 0.15s cubic-bezier(0.34,1.56,0.64,1)' }}
            ...
            <Tooltip text={note.isPinned ? 'Unpin' : 'Pin note'} position="top">
              <button
                ...
                style={{ width:24,height:24,display:'flex',alignItems:'center',justifyContent:'center',borderRadius:5,border:'none',background:'transparent',cursor:'pointer',fontSize:12,color:note.isPinned?'var(--warning)':'var(--text-tertiary)',transition:'color 0.15s ease, transform 0.15s cubic-bezier(0.34,1.56,0.64,1)' }}
                onMouseEnter={e => e.currentTarget.style.transform = 'scale(1.2)'}
                onMouseLeave={e => e.currentTarget.style.transform = 'scale(1)'}
              >◉</button>
            </Tooltip>
            <Tooltip text="Edit note" shortcut="E" position="top">
              <button
                onClick={e => stop(e, () => onEdit(note))}
                style={{ width:24,height:24,display:'flex',alignItems:'center',justifyContent:'center',borderRadius:5,border:'none',background:'transparent',cursor:'pointer',fontSize:12,color:'var(--text-tertiary)',transition:'all 0.15s ease' }}
...
export default function NotesGrid({ activeView, viewMode, onViewNote, onEditNote, onShareNote }) {
      ...
      return (
        <div>
          ...
          <div style={{ display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'60px 20px',gap:14,textAlign:'center' }}>
            ...
            <p style={{ fontSize:13.5,color:'var(--text-tertiary)',maxWidth:300,lineHeight:1.6 }}>Try adjusting or clearing your filters.</p>
            <div style={{ display:'flex',flexWrap:'wrap',justifyContent:'center',gap:8,marginTop:4 }}>
              {activePills.map(pill => (
                <span key={pill.key} style={{ display:'inline-flex',alignItems:'center',gap:6,background:'var(--bg-card)',border:'1px solid var(--border-soft)',color:'var(--text-secondary)',fontSize:12,padding:'5px 12px',borderRadius:20,boxShadow:'var(--shadow-xs)' }}>
                  {pill.label}
  ...
  return (
    <div>
      ...
      <div className={gridCls}>{regular.map((n,i)=><NoteCard key={n._id} {...cardProps(n)} index={pinned.length ? i : i}/>)}</div>
    </div>
  )
}
