...
router.get('/:token', async (req, res) => {
  try {
    const note = await Note.findOne({
      ...
      isShared: true,
      isTrashed: false,
    }).select('title content tags subject priority color wordCount readTime createdAt updatedAt sharedAt');
...
router.post('/:id/share', protect, async (req, res) => {
  try {
    ...
    res.json({ success: true, data: { shareToken: token, sharedAt: note.sharedAt } });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
});
